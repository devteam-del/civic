...
驗證日期：2026-09-10T18:41:04。本次驗證已出具結果，**整體模型尚未通過最終驗收**。

目前模型：`/Users/ktlu/Desktop/Civic_Rebuild_20260909_201934/Validation/CIVIC_VALIDATION_REPAIRED_20260910_183657.blend`。原始模型、修正前完整快照與被替換網格皆保留。GitHub 備份的是指令及報告，`.blend` 完整檔在本機。

## 本次重跑結果

| 檢查 | 結果 | 範圍及證據 |
|---|---|---|
| 基礎網格 | 主場景通過本次指標 | 6,317 個使用中的 mesh datablock；主渲染物件異常由 2 降為 0。全檔仍有 21 個保留來源／舊模型／平面網格標記，未宣告全檔封閉。見 technical_audit.json。 |
| 修改器輸出 | **未通過** | 道路 3 條、分隔島 2 條多面共邊；兩者邊界邊與零面積面均為 0。這些布林結果不包含在基礎網格的通過結論內。 |
| 地下入口行走抽樣 | 通過指定路徑 | 19 個直梯比較入口＋Y26 B 整合版，共 801 個位置、20,025 條障礙射線，碰撞 0；中心腳底缺乏支撐 0。含屋頂與渲染隱藏玻璃，排除封存舊件。 |
...
| 來源標记 | 已盤點，非來源真實性認證 | 5636 個網格物件有來源／假設／高度等屬性。其餘須參照集合與階段報告；未逐筆證實現地。 |
