...
root=pathlib.Path('/Users/ktlu/Desktop/Civic_Rebuild_20260909_201934');out=root/'FrontageSolids';out.mkdir(exist_ok=True)
...
p=json.load(open('/tmp/civic-frontage-solids/payload.json'));col=bpy.data.collections.new('FRONTAGE_ZHONGLIN_HEIGHT_COMPARISON');main.collection.children.link(col)
mats=[]
...
sc=bpy.data.scenes.new('FRONTAGE_HEIGHT_COMPARISON');sc.collection.children.link(col)
...
camd=bpy.data.cameras.new('FRONTAGE_HEIGHT_CAMERA');cam=bpy.data.objects.new(camd.name,camd);sc.collection.objects.link(cam);sc.camera=cam;cam.location=(1250,370,240);cam.rotation_euler=(Vector((1210,730,15))-cam.location).to_track_quat('-Z','Y').to_euler();camd.type='ORTHO';camd.ortho_scale=430;camd.clip_end=2000
...
result={'file':str(file),'solids':report,'unknown_height_candidates_retained_in_original_context':43,'new_mesh_nonmanifold_edges':0,'main_context_replaced':False,'limitations':'13 candidate solids in separate comparison scene; not full-corridor completion. 12 heights assumed; one OSM height. Original context retained without overlap in main scene.'};(out/'frontage_check.json').write_text(json.dumps(result,ensure_ascii=False,indent=2))
