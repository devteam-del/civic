"""Full-corridor frontage candidates by first polygon intersected by street-normal rays.
...
summary={'candidate_buildings':len(records),'explicit_height':sum(r['height_m'] is not None for r in records),'levels_only':sum(r['height_m'] is None and r['levels'] is not None for r in records),'no_height_or_levels':sum(r['height_m'] is None and r['levels'] is None for r in records),'sampling_sensitive':sum(not r['stable_at_5m'] for r in records),'search_depth_sensitive':sum(not r['stable_at_100m'] for r in records),'axis_length_m':axis.length,'scope':'Full derived Civic axis; simple OSM way footprints; relations may be missing'}
...
xx,yy=zip(*map(local,axis.coords));ax.set_xlim(min(xx)-25,max(xx)+25);ax.set_ylim(min(yy)-150,max(yy)+150);ax.set_aspect('equal');ax.grid(alpha=.15);ax.set_title('CIVIC FULL CORRIDOR | first-row building candidates and height evidence');ax.set_xlabel('Blender local X (m)');ax.set_ylabel('Blender local Y (m)')
