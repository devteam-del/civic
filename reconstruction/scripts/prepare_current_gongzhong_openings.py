...
for r in rows:
 ...
 source=top_poly(r);oldarea=source.area;own=r['name'] in ['COMP_公中_B1_FLOOR','COMP_公中_B2_FLOOR','COMP_ROOF_ACCESS_公中_0'];filled=source.union(oldhole) if own else source;remain=filled if r['name']=='COMP_公中_B2_FLOOR' else filled.difference(newhole)
 ...
 payload.append({'source':r['name'],'vertices':vs,'faces':fs,'old_area_m2':oldarea,'new_area_m2':remain.area,'old_core_hole_filled':own,'new_hole_overlap_removed_m2':0 if r['name']=='COMP_公中_B2_FLOOR' else filled.intersection(newhole).area,'bottom_floor_closed':r['name']=='COMP_公中_B2_FLOOR'})
