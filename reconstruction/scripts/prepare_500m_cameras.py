"""500m chainage on a derived boulevard analysis axis, not official engineering chainage.
...
root=pathlib.Path('/Users/ktlu/Desktop/Civic_Rebuild_20260909_201934');out=root/'Cameras_500m';out.mkdir(exist_ok=True)
...
line=LineString(axis);stations=[float(s) for s in range(0,int(line.length)+1,500)]
...
for i,s in enumerate(stations):
 ...
 rows.append({'index':i,'chainage_m':s,'label':tag,'section':t['name'],'twd97':[p.x,p.y],'lonlat':[lon,lat],'live_xy':q,'true_north_live_unit':[dx/d,dy/d],'regular_500m_station':s%500==0})
