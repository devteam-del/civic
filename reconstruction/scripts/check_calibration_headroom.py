...
out=pathlib.Path('/Users/ktlu/Desktop/Civic_Rebuild_20260909_201934/Calibration');sc=bpy.data.scenes['CIVIC_COMPLETE_WORKING_ASSEMBLY'];bpy.context.window.scene=sc;sc.view_layers[0].update();dg=bpy.context.evaluated_depsgraph_get();vv=[];ff=[];owner=[];roof=[];retained=set(o.name for o in bpy.data.collections['CALIBRATION_RETAINED_CONFLICT_OBJECTS'].objects);roofs=set(o.name for o in bpy.data.collections['COMPLETION_REMOVABLE_ROOFS'].objects)
for o in sc.objects:
 if o.type!='MESH' or o.name in retained:continue
 ...
 e.to_mesh_clear()
bvh=BVHTree.FromPolygons(vv,ff,all_triangles=False);paths=json.load(open(out/'checked_paths.json'));issues=[];samples=0
for p in paths:
...
r={'sampled_paths':len(paths),'samples':samples,'paths_with_obstructions':len(issues),'issues':issues,'method':'Upward rays from interpolated tread/ramp height+0.2m to assumed pedestrian2.0m/vehicle2.1m envelope; includes hidden cutaway roofs. Sampling only, not continuous clearance certification.'};(out/'headroom_check.json').write_text(json.dumps(r,ensure_ascii=False,indent=2));(out/'evaluated_roofs.json').write_text(json.dumps(roof,ensure_ascii=False));result={'paths':len(paths),'samples':samples,'blocked_paths':len(issues),'objects':dict(collections.Counter(h['object'] for i in issues for h in i['hits']))}
