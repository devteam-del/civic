...
ids=['w875067966','w1015818580'];rows=json.load(open(out/'corrected_rows_context.json'));items=[]
for r in rows:
 ...
 items.append({'osm_id':r['osm_id'],'pieces':pieces,'height_m':height,'prior_height_m':r['height_m'],'height_uncertainty_m':(1.0 if r['osm_id']=='w1015818580' else 2.0),'observed_group_member':r['osm_id']=='w1015818580'})
for row in items:
 for m in row['pieces']:
  edges={}
  for f in m['faces']:
...
r={'items':items,'source_url':'https://www.google.com/maps/@25.047999,121.5160201,3a,90y,270h,90t/data=!3m4!1e1!3m2!1sqKQ4Ga927cEaDMDG_3GRKg!2e0','imagery_date':'2025-03','scope':'w1015818580 identified beside the2025station group; height5.4m +/-1m estimated. w875067966 exterior not retrieved: nearest panorama was indoors and cannot verify its exterior. Its5.4m +/-2m model is explicitly inferred from nearby small utility shafts, replacing an inherited16.65m neighboring-building estimate. All rear faces and openings remain estimated; original proxies retained.'};(out/'additional_small_vent_photo_payload.json').write_text(json.dumps(r,ensure_ascii=False));print({'shafts':len(items),'pieces':sum(len(x['pieces']) for x in items)})
