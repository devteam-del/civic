...
out=pathlib.Path(os.environ['CIVIC_MODEL_ROOT'])/'CalibrationRound3';sc=bpy.data.scenes['CIVIC_BLOCKS_FACADES_WORKING'];bpy.context.window.scene=sc;r=json.load(open(out/'small_station_vent_photo_payload.json'));name='SMALL_STATION_VENT_SHELLS_0912_EST';assert not bpy.data.collections.get(name),'Already built';lookup={x['osm_id']:x['new_object'] for x in json.load(open(out/'mapped_vent_shell_report.json'))['items']};old=[sc.objects[lookup[x['osm_id']]] for x in r['items']]
bpy.ops.wm.save_as_mainfile(filepath=str(out/('BEFORE_SMALL_STATION_VENTS_'+datetime.datetime.now().strftime('%Y%m%d_%H%M%S')+'.blend')),copy=True)
for col in list(sc.collection.children):
 targets=[o for o in old if o.name in col.objects]
 if not targets:continue
 state=sc.view_layers[0].layer_collection.children[col.name].exclude;clone=col.copy();clone.name='SMALL_STATION12_'+col.name;sc.collection.children.unlink(col);sc.collection.children.link(clone)
...
report={'items':report,'source_url':r['source_url'],'imagery_date':r['imagery_date'],'scope':r['scope']};(out/'small_station_vent_photo_report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
review=bpy.data.scenes.new('SMALL_STATION_VENT_REVIEW_0912');review.use_fake_user=True;review.collection.children.link(col);review.render.engine='BLENDER_WORKBENCH';review.render.resolution_x=1200;review.render.resolution_y=800;review.render.resolution_percentage=100;review.display.shading.light='STUDIO';review.display.shading.color_type='MATERIAL';review.display.shading.show_shadows=False;review.display.shading.show_cavity=True
cd=bpy.data.cameras.new('SMALL_STATION_VENT_REVIEW_CAMERA');cam=bpy.data.objects.new(cd.name,cd);review.collection.objects.link(cam);review.camera=cam;cd.type='ORTHO';cd.ortho_scale=40;cd.clip_end=500
for label,pos,target in [('GROUP',(568,700,27),(522,760,2.7)),('FRONT',(522,706,15),(522,760,2.7))]:
 cam.location=pos;cam.rotation_euler=(Vector(target)-cam.location).to_track_quat('-Z','Y').to_euler();review.render.filepath=str(out/('SMALL_STATION_VENTS_'+label+'.png'));bpy.ops.render.render(write_still=True,scene=review.name)
