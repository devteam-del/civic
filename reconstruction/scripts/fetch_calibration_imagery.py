...
root=pathlib.Path('/Users/ktlu/Desktop/Civic_Rebuild_20260909_201934');out=root/'Calibration';out.mkdir(exist_ok=True);tiles=out/'tiles';tiles.mkdir(exist_ok=True);(out/'stations').mkdir(exist_ok=True);(out/'source').mkdir(exist_ok=True)
...
rows=json.load(open(root/'Cameras_200m/camera_stations.json'))['stations'];z=18;N=2**z
...
for r in rows:
 ...
 px=(x-ix+1)*256;py=(y-iy+1)*256;d=ImageDraw.Draw(im);d.ellipse((px-5,py-5,px+5,py+5),outline='red',width=2);d.text((8,776),r['label']+' | NLSC PHOTO2 | acquisition date unknown',fill='black');im.save(out/'stations'/(r['label']+'.jpg'));r['image']=str(out/'stations'/(r['label']+'.jpg'));r['pixel_center']=[px,py];r['tile_origin']=[ix-1,iy-1];r['zoom']=z
manifest={'source':'NLSC PHOTO2 orthophoto, not satellite-specific sensor; do not infer current acquisition date from retrieval time','service':'https://maps.nlsc.gov.tw/S09SOA/pro/Wmts_ajax_main.jsp','zoom':z,'stations':rows,'tiles':results,'fetched_at':time.strftime('%Y-%m-%dT%H:%M:%S'),'failures':sum(not r['ok'] for r in results)};(out/'imagery_manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2));print({'tiles':len(results),'failures':manifest['failures'],'station_images':len(rows)})
