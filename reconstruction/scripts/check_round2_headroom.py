...
out=pathlib.Path('/Users/ktlu/Desktop/Civic_Rebuild_20260909_201934/CalibrationRound2');sc=bpy.data.scenes['CIVIC_BLOCKS_FACADES_WORKING'];bpy.context.window.scene=sc;sc.view_layers[0].update();dg=bpy.context.evaluated_depsgraph_get();vv=[];ff=[];owner=[];roof=[];retained=set(o.name for o in bpy.data.collections['CALIBRATION_RETAINED_CONFLICT_OBJECTS'].objects);roofs=set(o.name for o in bpy.data.collections['COMPLETION_REMOVABLE_ROOFS'].objects)
for o in sc.objects:
 ...
 e.to_mesh_clear()
bvh=BVHTree.FromPolygons(vv,ff,all_triangles=False);paths=json.load(open(out.parent/'Calibration/checked_paths.json'));issues=[];samples=0
for p in paths:
