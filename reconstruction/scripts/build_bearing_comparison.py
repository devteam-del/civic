...
for cap in source.objects:
 ...
 for girder,points in hits.items():
  # Pick the sample nearest group centroid whose entire 0.5x0.5 footprint fits both solids.
  ...
  if chosen is None:skipped.append({'cap':cap.name,'girder':girder,'reason':'0.5m square footprint did not fit sampled cap/flange intersection'});continue
  q=chosen;ident=cap.name+'_'+str(added)
  # 0.025 steel / 0.15 core / 0.025 steel = selected0.20m stack.
  cube('BEARING_BASE_'+ident,q.x,q.y,top+.0125,.5,.5,.025,steel)
  cube('BEARING_CORE_'+ident,q.x,q.y,top+.1,.45,.45,.15,rubber)
  cube('BEARING_TOP_'+ident,q.x,q.y,top+.1875,.5,.5,.025,steel)
  bearing_rows.append({'cap':cap.name,'girder':girder,'xy':[q.x,q.y],'bottom_z':top,'top_z':top+.2,'original_girder_underside_z':hi[2],'gap_error_m':abs(top+.2-hi[2]),'footprint_m':[.5,.5],'corner_contact_check':'four sampled corners fit cap and flange'});added+=1
