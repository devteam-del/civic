...
jobs=[('p234_missing_north_review.json','apply_missing_support_observations.py'),('p233_position_review.json','apply_observation_set_xy.py'),('west233_position_review.json','apply_observation_set_xy.py'),('west233_missing_south_review.json','apply_missing_support_observations.py'),('antong_west_position_review.json','apply_observation_set_xy.py'),('huaisheng_position_review.json','apply_observation_set_xy.py'),('huaisheng_west_position_review.json','apply_observation_set_xy.py'),('huaisheng_footbridge_position_review.json','apply_observation_set_xy.py')]
checks=[]
for filename,script in jobs:
 records=json.loads((root/filename).read_text())['targets']
 if all(bool(r.get('comparison_object')) and r['comparison_object'] in scene.objects for r in records):
  checks.append({'file':filename,'already_present':True});continue
 assert not any(bool(r.get('comparison_object')) and r['comparison_object'] in scene.objects for r in records),'Partial application needs review'
