...
d=json.loads((root/'fuxing_west_ground_comparison.json').read_text())
replacements={};checks=[]
for patch in d['patches']:
 if patch['new_name'] in scene.objects:continue
 source_name=patch['source_object']
 if source_name=='SV_FUXING_WEST_MEDIAN_EST' and source_name not in scene.objects:source_name='CAL_GROUND_MEDIAN_WORKING_ESTIMATED_9'
 old=scene.objects[source_name]
 ...
 ob['source_object']=old.name;ob['source_record']='fuxing_west_ground_comparison.json'
...
(root/'fuxing_west_ground_application.json').write_text(json.dumps(report,indent=2))
result=report
