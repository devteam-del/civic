...
d=json.loads((root/'dunhua_median_comparison.json').read_text())
replacements={};checks=[]
for patch in d['patches']:
 ...
 ob['source_object']=old.name;ob['source_record']='dunhua_median_comparison.json'
...
(root/'dunhua_median_application.json').write_text(json.dumps(report,indent=2))
result=report
