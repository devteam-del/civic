...
old=shape(next(p for p in prior['patches'] if p['new_name']=='SV_P240_P241_MEDIAN_EST')['footprint'])
...
ws=south[0][1]-2;wn=north[0][1]+2
es,en=old.intersection(LineString([(3490,350),(3490,480)])).bounds[1::2]
profile=Polygon([(3404,ws)]+[(x,y-2) for x,y in south]+[(3490,es),(3490,en)]+[(x,y+2) for x,y in reversed(north)]+[(3404,wn)])
opening=Polygon()
new=unary_union([old.difference(box(3404,350,3490,480)),profile]).difference(holes).difference(opening)
assert new.is_valid
roadpatch=next(p for p in prior['patches'] if p['new_name']=='SV_P240_P241_ROAD_EST')
...
patches=[p for p in prior['patches'] if p['new_name'] not in ['SV_P240_P241_MEDIAN_EST','SV_P240_P241_ROAD_EST']]
for source,name,g,z0,z1 in [('SV_P240_P241_MEDIAN_EST','SV_P237_P239_MEDIAN_EST',new,0,.18),('SV_P240_P241_ROAD_EST','SV_P237_P239_ROAD_EST',road,-.25,0)]:
...
out=dict(prior);out.update(patches=patches,omit_from_working_scene=prior['omit_from_working_scene'],p237_p239_checks=checks,p237_p239_openings={'footprint':mapping(opening),'status':'No additional gap introduced inside this local segment. Western median end3404 is a proportional estimate from visible island nose, not surveyed.','panos':['W7yVqpmB3_GRMjErCguLjQ','xZ2nr-a3fba9r6XAYhYdcQ','qO9vy2z0l826vmFLb83htg'],'satellite_context':'No independent satellite curb measurement; bridge deck occludes curbs.'},assumptions=prior['assumptions']+' P237-P239 profile uses2m shaft offsets; western island nose x3404 estimated from visible context, not measured. Existing holes retained.')
