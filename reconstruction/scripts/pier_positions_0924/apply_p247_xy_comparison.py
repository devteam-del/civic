"""Apply P247 XY comparison only; retain original scenes and shaft elevations."""
import bpy,json,math
from pathlib import Path
from mathutils import Vector
root=Path(bpy.data.filepath).parent/'PierPositions_20260924'
d=json.loads((root/'p247_position_review.json').read_text())
scene=bpy.context.scene
assert scene.name=='CIVIC_PIER_XY_REVIEW_20260923'
assert bpy.data.objects.get('SV_XY_EST_P247_MODEL732') is None
...
ob=old.copy();ob.data=old.data.copy();ob.name='SV_XY_EST_P247_MODEL732'
ob.matrix_world.translation+=Vector((*d['delta_xy'],0))
ob['source_object']=old.name;ob['visible_pier_label']='P247'
ob['source_record']='p247_position_review.json'
...
def clone(c):
 ...
 nc=bpy.data.collections.new(c.name+'_P247_XY');memo[c]=nc
...
assert z0==z1
col=bpy.data.collections.new('P247_STREETVIEW_XY_EVIDENCE');scene.collection.children.link(col)
for name,p,size in [('P247_OLD_MODEL732',d['original_xy'],1.),('P247_XY_EST_NOT_SURVEY',d['candidate_model_xy'],1.)]:
...
for i,v in enumerate(d['views']):
 curve=bpy.data.curves.new('P247_BEARING_'+str(i),'CURVE');curve.dimensions='3D';curve.bevel_depth=.02
...
(root/'p247_position_review.json').write_text(json.dumps(d,ensure_ascii=False,indent=2))
txt=bpy.data.texts.new('P247_XY_REVIEW');txt.write(json.dumps(d,ensure_ascii=False,indent=2))
