...
west=geo(next(o for o in src['medians'] if o['name']=='CAL_GROUND_MEDIAN_WORKING_ESTIMATED_11'))
east=shape(next(p for p in prior['patches'] if p['new_name']=='SV_DUNHUA_WEST_MEDIAN_EST')['footprint'])
old=unary_union([west,east])
...
ws,wn=old.intersection(LineString([(3548,350),(3548,480)])).bounds[1::2]
es,en=old.intersection(LineString([(3662,350),(3662,480)])).bounds[1::2]
profile=Polygon([(3548,ws)]+[(x,y-2) for x,y in south]+[(3662,es),(3662,en)]+[(x,y+2) for x,y in reversed(north)]+[(3548,wn)])
opening=unary_union([box(3600,350,3616,480),box(3630,350,3644,480)])
new=unary_union([old.difference(box(3548,350,3662,480)),profile]).difference(holes).difference(opening)
assert new.is_valid
roadpatch=next(p for p in prior['patches'] if p['new_name']=='SV_YANJI_ROAD_OPENINGS_EST')
...
patches=[p for p in prior['patches'] if p['new_name'] not in ['SV_DUNHUA_WEST_MEDIAN_EST','SV_YANJI_ROAD_OPENINGS_EST']]
for source,name,g,z0,z1 in [('SV_DUNHUA_WEST_MEDIAN_EST','SV_FUDUN_WEST_MEDIAN_EST',new,0,.18),('SV_YANJI_ROAD_OPENINGS_EST','SV_FUDUN_WEST_ROAD_EST',road,-.25,0)]:
...
out=dict(prior);out.update(patches=patches,omit_from_working_scene=[*prior['omit_from_working_scene'],'CAL_GROUND_MEDIAN_WORKING_ESTIMATED_11'],fudun_west_checks=checks,fudun_west_openings={'footprint':mapping(opening),'status':'Proportional crossing and turning gaps inferred from views around P243-P245. Boundaries are NOT surveyed.','panos':['ZSfVnofhfQk6MkCSSPibKQ','V1Ms-Pwo1NGzX82d62EMhA'],'satellite_context':'North Bade Rd Sec2 Lane366 and south Civic Blvd Sec4 Lane48; bridge deck obscures actual underbridge curbs.'},assumptions=prior['assumptions']+' Fudun west profile uses2m shaft offsets; existing parking ramp holes retained; crossing/turning gaps remain proportional estimates.')
