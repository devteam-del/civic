...
es,en=old.intersection(LineString([(4680,300),(4680,450)])).bounds[1::2]
profile=Polygon([(4457,ws)]+[(x,y-2) for x,y in south]+[(4680,es),(4680,en)]+[(x,y+2) for x,y in reversed(north)]+[(4457,wn)])
new=unary_union([old.difference(box(4457,300,4680,450)),profile]).difference(unary_union(holes))
vehicle_opening=box(4650,350,4666,450)
...
patches.append(dict(source_object='SV_RAMP_EAST_MEDIAN_EST',new_name='SV_RAMP_EAST_MEDIAN_EST_V2',footprint=mapping(new),vertices=v,faces=f))
checks=[]
for r in obs['targets']:
 ...
 checks.append(dict(pier=r['model_pier_candidate'],outside_median_m2=p.difference(new).area,vehicle_opening_overlap_m2=p.intersection(vehicle_opening).area,preserved_opening_overlap_m2=p.intersection(unary_union(holes)).area))
out=dict(prior);out['vehicle_opening_estimate']={'footprint':mapping(vehicle_opening),'evidence_pano':'SRliI6cnVt8aqMGLY_raqg','note':'Visible vehicle opening;16m longitudinal width is proportional estimate, not measured. Preserved before rebuilding curb envelope.'};out.update(patches=patches,omit_from_working_scene=[*prior['omit_from_working_scene'],'CAL_GROUND_MEDIAN_WORKING_ESTIMATED_17'],ramp_east_checks=checks,ramp_east_scope_x=[4457,4680],assumptions=prior['assumptions']+' Additional ramp-east curb envelope uses2m shaft offsets based on visible planted strip proportions. Preserve prior local crossing and existing holes. Not independent field clearance proof.')
