...
d=json.loads((root/'east_corridor_median_comparison.json').read_text())
replacements={};checks=[]
for patch in d['patches']:
 ...
 ob['source_object']=old.name;ob['source_record']='east_corridor_median_comparison.json'
...
for name in d['omit_from_working_scene']:
 if name in scene.objects:replacements[scene.objects[name]]=None
flags={}
def read_flags(lc):
...
(root/'east_corridor_median_application.json').write_text(json.dumps(report,indent=2))
result=report
