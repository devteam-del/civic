...
root=(pathlib.Path(os.environ['CIVIC_MODEL_ROOT'])/'CalibrationRound3');out=root/'camera_renders';out.mkdir(exist_ok=True);sc=bpy.data.scenes['CIVIC_BLOCKS_FACADES_WORKING'];baseline_names={r['name'] for r in json.load(open(root.parent/'Calibration/camera_review_index.json'))};cams=sorted([o for o in sc.objects if o.type=='CAMERA' and o.name in baseline_names],key=lambda o:o.name);start=CAL_START;end=CAL_END
oldcam=sc.camera;oldengine=sc.render.engine;oldx=sc.render.resolution_x;oldy=sc.render.resolution_y;oldpercent=sc.render.resolution_percentage;oldpath=sc.render.filepath;oldtransparent=sc.render.film_transparent;oldshadow=sc.display.shading.show_shadows
rows=[]
try:
 ...
 for o in cams[start:end]:
  sc.camera=o;sc.display.shading.show_shadows=o.name.startswith('CIVIC200_');sc.view_layers[0].update();p=out/(o.name+'.png');sc.render.filepath=str(p);bpy.ops.render.render(write_still=True,scene=sc.name);rows.append({'name':o.name,'file':str(p),'location':list(o.location),'rotation_euler':list(o.rotation_euler),'lens_mm':o.data.lens,'sensor_width_mm':o.data.sensor_width,'horizontal_fov_deg':math.degrees(2*math.atan(o.data.sensor_width/(2*o.data.lens))),'scope':'Model visual review only; streetview comparison not completed when imagery unavailable'})
finally:
 sc.camera=oldcam;sc.render.engine=oldengine;sc.render.resolution_x=oldx;sc.render.resolution_y=oldy;sc.render.resolution_percentage=oldpercent;sc.render.filepath=oldpath;sc.render.film_transparent=oldtransparent;sc.display.shading.show_shadows=oldshadow
