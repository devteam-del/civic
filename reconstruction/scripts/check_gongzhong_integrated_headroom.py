import bpy,os
sc=bpy.data.scenes['GONGZHONG_INTEGRATED_OPENINGS_EST'];bpy.context.window.scene=sc
states=[]
def reveal_roofs(lc):
 if lc.collection.name=='GZI_COMPLETION_REMOVABLE_ROOFS':states.append((lc,lc.exclude));lc.exclude=False
...
try:
 ...
 out=(pathlib.Path(os.environ['CIVIC_MODEL_ROOT'])/'CalibrationRound3');sc=bpy.data.scenes['GONGZHONG_INTEGRATED_OPENINGS_EST'];bpy.context.window.scene=sc;sc.view_layers[0].update();dg=bpy.context.evaluated_depsgraph_get();vv=[];ff=[];owner=[];roof=[];retained=set(o.name for o in bpy.data.collections['CALIBRATION_RETAINED_CONFLICT_OBJECTS'].objects);roofs=set(o.name for o in bpy.data.collections['GZI_COMPLETION_REMOVABLE_ROOFS'].objects)
 for o in sc.objects:
