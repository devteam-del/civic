...
previous=json.load(open(os.path.join(root,'support_alignment_build_report.json'))) if existing else {'records':[]}
already={r['pier'] for r in previous['records']}
...
plans=[]
for r in a['groups']:
 if r['status']!='CONSTRAINED_ESTIMATE_NOT_ADOPTED' or any(n in already for n in r['piers']):continue
 ...
 # Bearings sit on the cap where girders cross, not necessarily over shaft axes.
 clo,chi=bound(cap);contacts=[]
 for ix in range(13):
  for iy in range(25):
   x=clo[0]+.35+(chi[0]-clo[0]-.7)*ix/12+dx
   y=clo[1]+.35+(chi[1]-clo[1]-.7)*iy/24+dy
   h=ray(beams,x,y,True)
   if h is None:continue
   valid=True
   for ox,oy in [(-.3,-.3),(-.3,.3),(.3,-.3),(.3,.3)]:
    caphit=ray([(cap,clo,chi)],x+ox-dx,y+oy-dy,False)
    bh=ray(beams,x+ox,y+oy,True)
    if caphit is None or bh is None or abs(bh[0]-h[0])>.03:valid=False;break
   if valid:contacts.append((x,y,h))
 chosen=[]
 for v in items:
  options=[p for p in contacts if all(math.hypot(p[0]-q[0],p[1]-q[1])>.9 for q in chosen)]
  if options:
   pick=min(options,key=lambda p:math.hypot(p[0]-v['xy'][0],p[1]-v['xy'][1]));chosen.append(pick)
   v['bearing_xy']=[pick[0],pick[1]];v['beam']=pick[2]
  else:v['beam']=None
...
replacements={};newobjects=[];records=list(previous['records'])
for r,cap,items,bottom,top in plans:
 ...
 for v in items:
  ...
  records.append({'pier':old.name,'replacement':ob.name,'delta_xy':r['delta_xy'],'ground_z':v['ground'][0],'shaft_top_z':bottom,'shaft_height_m':bottom-v['ground'][0],'beam_z':v['beam'][0],'pad_thickness_m':.2,'bearing_xy':v['bearing_xy'],'road_overlap_after_xy_m2':r['new_road_outside_median_m2'],'real_world_verified':False})
  # Work-model bearing pad at the pier axis, not an observed bearing footprint.
  x,y=v['bearing_xy'];z=top
  vs=[(x+sx*.3,y+sy*.3,z+sz*.2) for sz in [0,1] for sy in [-1,1] for sx in [-1,1]]
  ...
  pad=bpy.data.objects.new(old.name+'_PAD_EST',me);pad['basis']='0.6m square x 0.2m model bearing; dimensions unverified';newobjects.append(pad)
...
scene=existing or source.copy();scene.name=name
memo={}
...
pads=bpy.data.collections.get('ALIGN_EST_BEARINGS')
if pads is None:pads=bpy.data.collections.new('ALIGN_EST_BEARINGS');scene.collection.children.link(pads)
for o in newobjects:
