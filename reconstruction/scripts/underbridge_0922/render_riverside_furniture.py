...
s=bpy.data.scenes.new('UB_RIVERSIDE_FURNITURE_REVIEW')
s.collection.children.link(bpy.data.collections['UB_RIVERSIDE_FURNITURE_EST'])
for o in bpy.data.scenes['CIVIC_UNDERBRIDGE_MASSING_20260916'].objects:
 ...
 if -500<x<-400 and any(k in o.name for k in ['MEDIAN_WORKING','COMP_ACCESS','VENT_DETAIL','COMP_MEDIAN','UB_LINSEN','Pier_']):s.collection.objects.link(o)
cam=bpy.data.objects.new('UB_LAND_REVIEW_CAMERA',bpy.data.cameras.new('UB_LAND_REVIEW_CAMERA'));s.collection.objects.link(cam);cam.data.type='ORTHO';cam.data.ortho_scale=35;s.camera=cam
...
for tag,x,y in [('RIVERSIDE_FURNITURE',-447,1048)]:
