...
import bpy,json,os,pathlib,math,collections,datetime
...
markers=sorted([m for m in sc.timeline_markers if m.name.startswith('EW_1SEC_')],key=lambda m:m.frame);assert len(markers)==106 and len({m.camera.name for m in markers})==106;assert all(b.frame-a.frame==24 for a,b in zip(markers,markers[1:]));sc.frame_set(1)
...
path=out/'CIVIC_FULL_CORRIDOR_WORKING_20260912.blend';bpy.ops.wm.save_as_mainfile(filepath=str(path));(out/'checkpoint_manifest_0912.json').write_text(json.dumps({'file':str(path),'saved_at':datetime.datetime.now().isoformat(),'size_bytes':path.stat().st_size,'main_scene':sc.name,'scope':r['limitations']},ensure_ascii=False,indent=2));result={'photo_or_inferred_vents':len(records),'meshes_checked':meshes,'errors':len(errors),'remaining_generic_vents':len(remaining),'file':path.name}
