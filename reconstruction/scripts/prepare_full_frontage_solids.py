...
d=json.load(open('/tmp/civic-frontage-full/first_row.json'));rows=[]
...
out=pathlib.Path('/tmp/civic-frontage-full/solids');out.mkdir(exist_ok=True)
(out/'payload.json').write_text(json.dumps({'buildings':rows,'unknown_retained':len(d['buildings'])-len(rows),'scope':'Full derived Civic axis; independent comparison, source context retained'},ensure_ascii=False))
print({'solids':len(rows),'unknown':len(d['buildings'])-len(rows)})
