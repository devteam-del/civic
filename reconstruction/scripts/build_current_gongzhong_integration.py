...
out=pathlib.Path(os.environ['CIVIC_MODEL_ROOT'])/'CalibrationRound3';source=bpy.data.scenes['CIVIC_BLOCKS_FACADES_WORKING'];name='GONGZHONG_INTEGRATED_OPENINGS_0912_EST';assert not bpy.data.scenes.get(name),'Already built';payload=json.load(open(out/'gongzhong_integrated_openings_payload.json'));
# Reject stale opening payloads before modifying the scene.
reference={r['name']:r for r in json.load(open(out/'gongzhong_integration_surfaces.json'))};lc=source.view_layers[0].layer_collection.children['COMPLETION_REMOVABLE_ROOFS'];was=lc.exclude;lc.exclude=False;source.view_layers[0].update()
try:
 for row in payload['items']:
  obj=bpy.data.objects[row['source']];prior=reference[obj.name]['vertices'];assert len(prior)==len(obj.data.vertices),obj.name
  assert max((obj.matrix_world@v.co-Vector(p)).length for v,p in zip(obj.data.vertices,prior))<.001,obj.name+' changed; export openings again'
finally:lc.exclude=was;source.view_layers[0].update()
bpy.ops.wm.save_as_mainfile(filepath=str(out/('BEFORE_GZ_INTEGRATED_'+datetime.datetime.now().strftime('%Y%m%d_%H%M%S')+'.blend')),copy=True)
scene=source.copy();scene.name=name;scene.use_fake_user=True;mapping={};states={lc.collection.name:lc.exclude for lc in source.view_layers[0].layer_collection.children}
for col in list(scene.collection.children):
 assert len(col.children)==0,'Nested collections need recursive isolation'
 clone=col.copy();clone.name='GZ12_'+col.name;scene.collection.children.unlink(col);scene.collection.children.link(clone);mapping[col.name]=clone
...
for o in list(source.objects):
 moving=o.name.startswith(('COMP_公中_CORE0_L','RAIL_CORE_公中_0_','GUARD_CORE_公中_0_','INFILL_CORE_公中_0_'));row=replace.get(o.name)
 if not moving and row is None:continue
 clone=o.copy();clone.name='GZ12_'+o.name
