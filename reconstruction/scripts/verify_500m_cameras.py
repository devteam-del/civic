...
root=pathlib.Path(ROOT);sc=bpy.context.scene;p=json.loads((root/'camera_build_check.json').read_text());before={'camera':sc.camera,'x':sc.render.resolution_x,'y':sc.render.resolution_y,'percentage':sc.render.resolution_percentage,'filepath':sc.render.filepath,'engine':sc.render.engine};checks=[];renders=[]
try:
 ...
 for r in p['cameras']:
  ...
  sc.camera=o;sc.render.filepath=str(root/(o.name+'.png'));bpy.ops.render.render(write_still=True);renders.append(sc.render.filepath)
finally:
 sc.camera=before['camera'];sc.render.resolution_x=before['x'];sc.render.resolution_y=before['y'];sc.render.resolution_percentage=before['percentage'];sc.render.filepath=before['filepath'];sc.render.engine=before['engine']
pairs={}
